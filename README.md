...
This repository contains the demonstration application for the **Track 2 model of the [2026 IEEE Low-Power Computer Vision Challenge (LPCVC)]** [1] running on physical mobile edge devices. The system has been validated on **Android 16** and **Samsung Galaxy S25 & S26** hardware.
...
The app comes pre-packaged with baseline assets configured out-of-the-box using tools adapted from the official [lpcvai/26LPCVC_Track2_Sample_Solution] pipeline:
...
* 🤖 `assets/models/model.dlc`  
  The converted Deep Learning Container (DLC) format optimized from ONNX for Snapdragon execution.  
  The model is adapted from the solution of the Track 2 first place winner, [EfficientAI's repo].  
  **Conversion script:** `onnx_to_dlc_inference.py`

* 📋 `assets/tutorial/label_videos.json`  
  A JSON lookup table linking label names to specific asset file paths.  
  **Generated by:** `select_example_videos.py`

* 🎥 `assets/tutorial/videos/`  
  A curated set of randomly selected videos from the training split of the QEVD dataset [2].
  **Generated by:** `select_example_videos.py`

  

* 🎥 `assets/tutorial/previews/`  
  Pre-rendered, 16-frame downsampled representations used for UI thumbnails.  
  **Generated by:** `create_example_thumbnails.py`
...
[lpcvai/26LPCVC_Track2_Sample_Solution]: https://github.com/lpcvai/26LPCVC_Track2_Sample_Solution

[EfficientAI's repo]:https://github.com/shuangtianxiaoye/LPCV-Track2-EfficientAI

[QEVD dataset]: https://www.qualcomm.com/developer/software/qevd-dataset

[Demo video link]: https://drive.google.com/file/d/1lV3henX-AZNv1-81DdW1k4DUY72mp7Xp/view?usp=sharing